...
const (
	...
	TLVMaetoContainer STAMPTLVType = 250
)

type SRExtensions struct {
	ReturnPath     *SRExtReturnPath
	MaetoContainer *SRExtMaetoContainer
}
...
func (p *SRExtensions) Encode() ([]byte, error) {
	...
	if p.MaetoContainer != nil {
		enc, err := p.MaetoContainer.Encode()
		if err != nil {
			return nil, err
		}
...
func DecodeSenderPacket(hmacKey []byte, b []byte) (*SenderPacket, error) {
	...
	for cursor < len(b) {
		...
		case TLVMaetoContainer:
			maetoContainer, err := decodeMaetoContainer(val)
			if err != nil {
				return nil, err
			}

			ext.MaetoContainer = maetoContainer
